...
const imageMap: Record<string, { src: string; alt: string }> = {
  "flow-measurement": { src: "/images/knowledge-centre/Flow Measurement.png", alt: "Industrial flow measurement instrumentation installed on process piping" },
  "level-measurement": { src: "/images/knowledge-centre/Level measurement.png", alt: "Industrial level measurement instrumentation and storage tank" },
  ...
  "process-analysis": { src: "/images/knowledge-centre/Process Analysis.png", alt: "Industrial process analysis instrumentation and process monitoring equipment" },
};
...
export default async function KnowledgeArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><PageHero eyebrow="Knowledge Centre" title={article.title} description={article.description} /><main className="py-20 sm:py-28"><div className="mx-auto max-w-5xl px-6 lg:px-10"><Reveal><Link href="/knowledge-centre" className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em] text-teal"><ArrowLeft size={14}/> Back to Knowledge Centre</Link></Reveal>
    {image ? <Reveal><figure className="mt-10 overflow-hidden rounded-2xl border border-line bg-surface"><img src={`${image.src}?v=6`} alt={image.alt} width={1800} height={1000} loading="eager" fetchPriority="high" className="block aspect-[16/9] w-full object-cover" /></figure></Reveal> : <Reveal><div className="mt-10 flex aspect-[16/9] w-full items-center justify-center rounded-2xl border border-dashed border-teal/30 bg-surface"><div className="text-center"><div className="font-mono text-xs uppercase tracking-[0.2em] text-teal">Image Placeholder</div><p className="mt-2 text-sm text-mute">Industrial photography will be added here</p></div></div></Reveal>}
