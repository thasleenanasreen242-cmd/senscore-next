...
function ArticleNavigation({ previous, next }: { previous?: (typeof KNOWLEDGE_ARTICLES)[number]; next?: (typeof KNOWLEDGE_ARTICLES)[number] }) {
  ...
  return <Reveal><nav aria-label="Knowledge Centre article navigation" className="mt-10 mb-20 grid gap-4 sm:grid-cols-2">{previous ? <Link href={`/knowledge-centre/${previous.slug}`} rel="prev" className={card}><ArrowLeft size={15} className="mb-3 text-teal transition-transform group-hover:-translate-x-1"/><span className="font-mono text-[10px] uppercase tracking-[0.16em] text-teal">Previous</span><div className="mt-1 font-display text-base text-ink">{previous.title}</div></Link> : <div aria-hidden="true" className="min-h-[116px]" />}{next ? <Link href={`/knowledge-centre/${next.slug}`} rel="next" className={`${card} sm:text-right`}><ArrowRight size={15} className="mb-3 ml-auto text-teal transition-transform group-hover:translate-x-1"/><span className="font-mono text-[10px] uppercase tracking-[0.16em] text-teal">Next</span><div className="mt-1 font-display text-base text-ink">{next.title}</div></Link> : <div aria-hidden="true" className="min-h-[116px]" />}</nav></Reveal>;
}
...
export default async function KnowledgeArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><PageHero eyebrow="Knowledge Centre" title={article.title} description={article.description} /><main className="py-20 sm:py-28"><div className="mx-auto max-w-5xl px-6 lg:px-10"><Reveal><Link href="/knowledge-centre" className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em] text-teal"><ArrowLeft size={14}/> Back to Knowledge Centre</Link></Reveal>
    {image ? <Reveal key={`knowledge-image-${slug}`}><div className="mt-10 aspect-[16/9] w-full overflow-hidden rounded-2xl border border-line bg-surface"><img src={encodeURI(image.src)} alt={image.alt} width={1800} height={1000} loading="eager" fetchPriority="high" decoding="async" className="block h-full w-full object-cover" /></div></Reveal> : null}
