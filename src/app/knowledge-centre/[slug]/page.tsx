...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      images: image ? [{ url: image.src, width: 1800, height: 1000, alt: image.alt }] : [],
    },
  };
}
...
export default async function KnowledgeArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const sections = article.sections.filter((section) => section !== help);
  const image = imageMap[article.slug];
          ...
          {image && (
            <Reveal>
              <div className="mt-10 overflow-hidden rounded-2xl border border-line bg-surface">
                <img src={`${image.src}?v=2`} alt={image.alt} width={1800} height={1000} loading="eager" fetchPriority="high" className="block aspect-[16/9] w-full object-cover" />
              </div>
            </Reveal>
          )}
            ...
            {sections.map((section, sectionIndex) => (
              <Reveal key={section.heading} delay={sectionIndex * 0.04}>
                <section className="rounded-2xl border border-line bg-surface p-7 sm:p-9">
                  ...
                  {section.bullets && <div className="mt-7 space-y-4">{section.bullets.map((bullet, i) => <div key={bullet} className="rounded-xl border border-line bg-void/30 p-5"><span className="font-mono text-xs text-teal">{String(i + 1).padStart(2, "0")}</span><p className="mt-2 text-sm leading-7 text-mute">{bullet}</p></div>)}</div>}
                </section>
              </Reveal>
            ))}
          </div>
