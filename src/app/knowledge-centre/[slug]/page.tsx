...
const images: Record<string,{src:string;alt:string}> = {
  ...
  "pressure-temperature":{src:"https://images.pexels.com/photos/35069659/pexels-photo-35069659.jpeg?auto=compress&cs=tinysrgb&w=1800",alt:"Industrial pressure gauge and process equipment for pressure measurement"},
  ...
  "flange-protection":{src:"https://images.pexels.com/photos/586019/pexels-photo-586019.jpeg?auto=compress&cs=tinysrgb&w=1800",alt:"Industrial steel pipelines, gauges and bolted pipe connections"},
  "compressed-air-energy-efficiency":{src:"https://images.pexels.com/photos/31257317/pexels-photo-31257317.jpeg?auto=compress&cs=tinysrgb&w=1800",alt:"Industrial compressed air compressor system with tanks and piping"},
  "engineering-practice":{src:"https://images.pexels.com/photos/36564992/pexels-photo-36564992.jpeg?auto=compress&cs=tinysrgb&w=1800",alt:"Industrial engineering automation equipment and process machinery"},
};

export function generateStaticParams(){return KNOWLEDGE_ARTICLES.map((article)=>({slug:article.slug}));}
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const article=getKnowledgeArticle(slug);if(!article)return{title:"Knowledge Centre | SensCore"};const image=images[article.slug];return{title:article.seoTitle,description:article.description,alternates:{canonical:`${baseUrl}/knowledge-centre/${article.slug}`},openGraph:{title:article.seoTitle,description:article.description,url:`${baseUrl}/knowledge-centre/${article.slug}`,type:"article",images:image?[{url:image.src,width:1800,height:1000,alt:image.alt}]:[]}};}
...
export default async function KnowledgeArticlePage({params}:{params:Promise<{slug:string}>}){
  ...
  const help=article.sections.find((section)=>section.heading.toLowerCase().startsWith("need help")); const sections=article.sections.filter((section)=>section!==help); const image=images[article.slug];
  return <><PageHero eyebrow="Knowledge Centre" title={article.title} description={article.description}/><main className="py-20 sm:py-28"><div className="mx-auto max-w-5xl px-6 lg:px-10">
    <Reveal><Link href="/knowledge-centre" className="inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.18em] text-teal"><ArrowLeft size={14}/> Back to Knowledge Centre</Link></Reveal>
    {image?<Reveal><figure className="mt-10 overflow-hidden rounded-2xl border border-line bg-surface"><img src={image.src} alt={image.alt} width={1800} height={1000} loading="eager" fetchPriority="high" className="h-auto max-h-[520px] w-full object-cover"/></figure></Reveal>:null}
