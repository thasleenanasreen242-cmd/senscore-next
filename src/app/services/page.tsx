...
const CORE_SERVICES: Service[] = [
  {
    ...
    description: "Establish actual flow conditions before selecting or replacing a flow meter. Using non-invasive flow measurement where technically suitable, we assess existing liquid, gas or utility lines to understand real operating flow rates and process conditions. The findings can help identify incorrectly sized or misapplied meters and provide measured data for instrument selection, system assessment and troubleshooting.",
    links: [
  ...
  },
  {
    ...
    description: "Understand where compressed air is generated, distributed, consumed and lost. SensCore assesses compressed air systems using actual operating data to identify efficiency losses, leakage and opportunities for performance improvement.",
    links: [
                    ...
                    <div className="flex items-start gap-5">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="mt-4 text-sm leading-7 text-mute">
                          Related: {service.links.map((link, n) => (
                            <span key={link.href}>
                              {n > 0 ? ", " : ""}
                              <Link href={link.href} className="text-teal underline-offset-4 hover:underline">
                                {link.label}
                              </Link>
                            </span>
                          ))}
                        ...
                        {service.subItems && (
                          <div className="mt-6 grid gap-4 sm:grid-cols-3">
                            {service.subItems.map((sub) => (
                              <div key={sub.label} className="rounded-xl border border-line bg-surface p-5">
                                <h4 className="font-mono text-xs uppercase tracking-[0.15em] text-teal">{sub.label}</h4>
                                <p className="mt-2 text-xs leading-relaxed text-mute">{sub.detail}</p>
                              </div>
                            ))}
                          </div>
                        )}
