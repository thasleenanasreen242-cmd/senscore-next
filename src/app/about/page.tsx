      ...
      <section className="border-b border-line py-24 sm:py-32">
        <div className="mx-auto grid max-w-7xl gap-16 px-6 lg:grid-cols-2 lg:px-10">
          <Reveal>
            ...
            <h2 className="mt-6 font-display text-3xl font-semibold leading-tight text-ink sm:text-4xl">Smarter Processes. Better Decisions.</h2>
          ...
          <Reveal delay={0.1}>
            <SectionEyebrow index="02" label="Engineering Solutions Built for Industry" />
            <p className="mt-6 text-base leading-relaxed text-mute sm:text-lg">SensCore began by supplying high-quality industrial instrumentation across the UAE. With a deep industry focus and a commitment to long-term impact, we grew into something clients relied on beyond the delivery note.</p>
            <p className="mt-4 text-base leading-relaxed text-mute sm:text-lg">We are not just a supplier. We are a strategic ally in navigating industrial challenges and unlocking future-ready solutions. That means reading the specification properly, selecting for the application rather than the price list, and staying accountable for the outcome.</p>
            <p className="mt-4 text-base leading-relaxed text-mute sm:text-lg">Today we combine trusted industrial equipment with automation and analytics, so the speed behind every proposal matches the engineering standard behind every selection.</p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="border-b border-line bg-surface py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <Reveal>
            ...
            <h2 className="mt-6 max-w-xl font-display text-3xl font-semibold leading-tight text-ink sm:text-4xl">Principles Behind Every Solution</h2>
          </Reveal>
          <div className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {VALUES.map((value, index) => {
              const Icon = value.icon;
              return <Reveal key={value.title} delay={index * 0.05}><div className="h-full rounded-2xl border border-line bg-surface p-7 transition-colors hover:border-teal/40"><span className="flex h-11 w-11 items-center justify-center rounded-lg border border-teal/30 bg-teal/5 text-teal"><Icon size={20} strokeWidth={1.75} /></span><h3 className="mt-6 font-display text-lg font-semibold text-ink">{value.title}</h3><p className="mt-2 text-sm leading-relaxed text-mute">{value.text}</p></div></Reveal>;
            })}
          </div>
        </div>
      </section>
