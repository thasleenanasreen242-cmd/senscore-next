    ...
    <>
      ...
      <section className="py-24 sm:py-32">
        <div className="mx-auto flex max-w-7xl flex-col gap-24 px-6 lg:px-10">
          {PRODUCT_CATEGORIES.slice(0, 6).map((cat, i) => {
            ...
            return <div key={cat.id} className={`grid items-start gap-12 lg:grid-cols-2 lg:gap-20 ${reversed ? "lg:[&>*:first-child]:order-2" : ""}`}>
              <Reveal>
                ...
                <h2 className="mt-6 font-display text-3xl font-semibold leading-tight text-ink sm:text-4xl">{cat.title}</h2>
          ...
          <Reveal>
            <SectionEyebrow index="07" label="SUSTAIN" />
            <h2 className="mt-6 font-display text-3xl font-semibold text-ink sm:text-4xl">Industrial Engineering Services</h2>
            <p className="mt-4 max-w-3xl text-base leading-relaxed text-mute sm:text-lg">On-site measurement, audits and engineering support for industrial facilities across the UAE, from flow and compressed-air performance to automation, equipment support and energy efficiency.</p>
          </Reveal>
        </div>
      </section>
