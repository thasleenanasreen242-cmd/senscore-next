    ...
    <>
      ...
      <section className="border-b border-line py-10 sm:py-14">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <Reveal delay={0.1}>
            <p className="mt-4 max-w-4xl text-base leading-relaxed text-mute sm:text-lg">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-16 px-6 lg:px-10">
          {INDUSTRIES.map((industry, index) => {
            ...
            if (badgeParts.length === 1) {
              const match = industry.badge.match(/^(\S+)\s+(.+)$/);
              if (match) {
                badgeParts[0] = match[1];
                badgeParts[1] = match[2];
              }
            }
            const footerTags = industry.footerTag.split(" · ");
            return (
              <Reveal key={industry.id} delay={index * 0.03}>
                <div className="group relative overflow-hidden rounded-2xl border border-line bg-surface/30 p-8 transition-all duration-500 hover:border-teal/25 hover:bg-surface/50 sm:p-12">
                  ...
                  <div className="relative border-t border-line pt-8 sm:pt-9">
                    <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
                      <div className="max-w-3xl">
                        <div className="flex items-center gap-3">
                          <span className="flex h-8 min-w-8 items-center justify-center rounded-full border border-teal/20 bg-teal/5 px-2 font-mono text-xs text-teal">
                      ...
                      <div className="inline-flex w-fit items-center gap-3 rounded-full border border-teal/30 bg-gradient-to-r from-teal/[0.10] to-transparent px-4 py-2.5 shadow-[0_0_24px_rgba(45,212,191,0.07)] transition-all duration-300 group-hover:border-teal/50 group-hover:shadow-[0_0_30px_rgba(45,212,191,0.12)]">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-teal/30 bg-void/60 text-teal">
                          <ShieldCheck size={14} strokeWidth={1.8} />
                        </span>
                        <span className="flex min-w-0 flex-col items-start gap-0.5 leading-tight">
                          <span className="font-mono text-sm font-bold tracking-[0.14em] text-teal">
                            {badgeParts[0]}
                          </span>
                          {badgeParts[1] && (
                            <span className="text-[10px] font-normal uppercase tracking-[0.12em] text-mute">
                              {badgeParts[1]}
                            </span>
                          )}
                        </span>
                    ...
                    <div className="mt-9 flex flex-wrap gap-2">
                      {footerTags.map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full border border-line bg-void/50 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.18em] text-faint transition-colors duration-300 group-hover:border-teal/20 group-hover:text-mute"
                        >
                          {tag}
                        </span>
                      ))}
      ...
      <section className="relative overflow-hidden py-28 sm:py-32">
        <div className="relative mx-auto max-w-2xl px-6 text-center">
          ...
          <Reveal delay={0.1}>
            <p className="mt-5 text-base text-mute">
              If your application involves measurement, automation, flow control, process analysis or industrial equipment, our engineers can review the requirement and help identify a technically suitable solution.
            </p>
          </Reveal>
          <Reveal delay={0.2}>
            <div className="mt-9">
