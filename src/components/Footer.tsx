    ...
    <footer className="relative border-t border-line bg-surface">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16 lg:px-10">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            <Link href="/" className="flex items-center gap-2.5">
              ...
              <span className="font-display text-lg font-semibold text-ink">SENS<span className="text-teal">CORE</span></span>
            </Link>
            <p className="mt-4 max-w-xs text-sm leading-relaxed text-mute">
              Engineered sensing, control, flow and industrial automation solutions for real operating environments in the UAE. Application-driven engineering, supported by intelligent systems built for faster, clearer decisions.
            </p>
            <div className="mt-6 flex items-center gap-3">
              <Link href={CONTACT.linkedin} target="_blank" className="inline-flex items-center gap-2 rounded-full border border-line px-3 py-1.5 text-mute transition-colors hover:border-teal/50 hover:text-teal"><Linkedin size={16} /><span className="text-sm">SensCore</span></Link>
              <Link href={CONTACT.instagram} target="_blank" className="inline-flex items-center gap-2 rounded-full border border-line px-3 py-1.5 text-mute transition-colors hover:border-teal/50 hover:text-teal"><Instagram size={16} /><span className="text-sm">@senscoretech</span></Link>
            </div>
          </div>

          <div>
            <div className="font-mono text-xs uppercase tracking-[0.2em] text-teal">Navigate</div>
            <ul className="mt-4 flex flex-col gap-3">
              {NAV_LINKS.filter((link) => link.label !== "Services").map((link) => (
                <li key={link.href}><Link href={link.href} className="text-sm text-mute transition-colors hover:text-ink">{link.label}</Link></li>
              ))}
              <li><Link href="/industrial-engineering-services" className="text-sm text-mute transition-colors hover:text-ink">Services</Link></li>
            </ul>
          </div>
        ...
        <div className="mt-14 flex flex-col items-center justify-center gap-4 border-t border-line pt-6 font-mono text-[11px] uppercase tracking-[0.15em] text-faint sm:flex-row">
          <span>System status: all sensors online</span><span className="hidden sm:inline">·</span><span>© {new Date().getFullYear()} SensCore. All rights reserved.</span>
        </div>
      </div>
    </footer>
  );
