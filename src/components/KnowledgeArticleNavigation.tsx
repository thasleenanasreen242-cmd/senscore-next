...
export default function KnowledgeArticleNavigation() {
  ...
  const card = "group min-h-[72px] rounded-xl border border-line bg-surface px-4 py-3 text-sm text-mute transition-colors hover:border-teal/40 hover:text-ink sm:px-4 sm:py-3";
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 pb-20 lg:px-10">
      <nav aria-label="Knowledge Centre article navigation" className="mt-10 grid gap-3 sm:grid-cols-2">
        {previous ? <Link href={`/knowledge-centre/${previous[0]}`} rel="prev" className={card}>
          <ArrowLeft size={13} className="mb-2 text-teal transition-transform group-hover:-translate-x-1" />
          <span className="font-mono text-[8px] uppercase tracking-[0.16em] text-teal">Previous</span>
          <div className="mt-0.5 font-display text-sm leading-5 text-ink">{previous[1]}</div>
        </Link> : <div aria-hidden="true" className="min-h-[72px]" />}
        {next ? <Link href={`/knowledge-centre/${next[0]}`} rel="next" className={`${card} sm:text-right`}>
          <ArrowRight size={13} className="mb-2 ml-auto text-teal transition-transform group-hover:translate-x-1" />
          <span className="font-mono text-[8px] uppercase tracking-[0.16em] text-teal">Next</span>
