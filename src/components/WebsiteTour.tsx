...
export default function WebsiteTour() {
  ...
  return <>
    {showRestart && !open && <button type="button" onClick={restart} aria-label="Restart website tour" className="fixed right-6 top-[205px] z-[90] inline-flex cursor-pointer items-center gap-2 rounded-full border border-teal/30 bg-[#0b121a]/90 px-4 py-2.5 font-mono text-[11px] uppercase tracking-[0.15em] text-teal shadow-[0_0_24px_rgba(45,212,191,0.12)] backdrop-blur-xl transition hover:border-teal/50 hover:bg-teal/10 max-sm:right-4 max-sm:top-[190px]"><Compass size={15} />Take the tour</button>}
    <AnimatePresence>
      {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] cursor-default" role="dialog" aria-modal="true" aria-labelledby="senscore-tour-title" aria-describedby="senscore-tour-description">
        ...
        <motion.div key={step} initial={{ opacity: 0, y: 14, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8 }} className="absolute bottom-6 left-1/2 z-[102] w-[calc(100%-2rem)] max-w-[430px] -translate-x-1/2 cursor-default rounded-2xl border border-teal/25 bg-[#0b121a]/96 p-5 shadow-[0_24px_80px_rgba(0,0,0,.5)] backdrop-blur-2xl sm:bottom-8 sm:p-6">
          <button type="button" onClick={close} aria-label="Close website tour" className="absolute right-3 top-3 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full text-mute hover:bg-white/5 hover:text-ink"><X size={17} /></button>
