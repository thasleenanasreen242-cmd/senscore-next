...
import { ArrowLeft, ArrowRight, Bot, Check, Compass, Layers3, MapPin, MessageCircle, MousePointer2, Sparkles, X } from "lucide-react";
...
export default function WebsiteTour() {
  ...
  return <>
    {showRestart && !open && <button type="button" onClick={restart} aria-label="Restart website tour" className="fixed right-6 top-[205px] z-[90] !cursor-pointer inline-flex items-center gap-2 rounded-full border border-teal/30 bg-[#0b121a]/90 px-4 py-2.5 font-mono text-[11px] uppercase tracking-[0.15em] text-teal shadow-[0_0_24px_rgba(45,212,191,0.12)] backdrop-blur-xl transition hover:border-teal/50 hover:bg-teal/10 max-sm:right-4 max-sm:top-[190px]" style={{ cursor: "pointer" }}><Compass size={15} />Take the tour</button>}
    <AnimatePresence>
      {open && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] !cursor-default" style={{ cursor: "default" }} role="dialog" aria-modal="true" aria-labelledby="senscore-tour-title" aria-describedby="senscore-tour-description">
        <div className="absolute inset-0 bg-[#02070b]/55 !cursor-default" style={{ cursor: "default" }} />
        ...
        <motion.div key={step} initial={{ opacity: 0, y: 14, scale: .98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8 }} className="absolute bottom-6 left-1/2 z-[102] w-[calc(100%-2rem)] max-w-[430px] -translate-x-1/2 !cursor-default rounded-2xl border border-teal/25 bg-[#0b121a]/96 p-5 shadow-[0_24px_80px_rgba(0,0,0,.5)] backdrop-blur-2xl sm:bottom-8 sm:p-6" style={{ cursor: "default" }}>
          <button type="button" onClick={close} aria-label="Close website tour" className="absolute right-3 top-3 flex h-9 w-9 !cursor-pointer items-center justify-center rounded-full text-mute hover:bg-white/5 hover:text-ink" style={{ cursor: "pointer" }}><X size={17} /></button>
          ...
          <div className="mt-5 flex items-center justify-between gap-3"><button type="button" onClick={close} className="!cursor-pointer rounded-lg px-2 py-2 text-xs text-mute hover:text-ink" style={{ cursor: "pointer" }}>Skip tour</button><div className="flex items-center gap-2"><button type="button" onClick={previous} disabled={step === 0} aria-label="Previous tour step" className="flex h-10 w-10 !cursor-pointer items-center justify-center rounded-lg border border-line text-mute hover:border-teal/40 hover:text-ink disabled:!cursor-not-allowed disabled:opacity-30" style={{ cursor: step === 0 ? "not-allowed" : "pointer" }}><ArrowLeft size={15} /></button><button type="button" onClick={next} className="inline-flex h-10 !cursor-pointer items-center gap-2 rounded-lg bg-teal px-4 text-xs font-semibold text-[#061015] hover:scale-[1.02]" style={{ cursor: "pointer" }}>{isLast ? <><Check size={14}/>Finish</> : <>Next <ArrowRight size={14}/></>}</button></div></div>
          <div className="mt-4 flex justify-between border-t border-line pt-3 font-mono text-[9px] uppercase tracking-[0.16em] text-faint"><span className="inline-flex items-center gap-1.5"><MousePointer2 size={11} /> cursor active</span><span>← → navigate · Esc closes</span></div>
        </motion.div>
      </motion.div>}
    </AnimatePresence>
  </>;
