...
export default function HUDBackground({density=7}:{density?:number}) {
  ...
  useEffect(()=>{
    ...
    function update(){nodes.forEach(n=>{
      if(pointerActive){const dx=n.x-pointerX,dy=n.y-pointerY,d=Math.hypot(dx,dy);if(d<pointerRadius&&d>.001){const s=1-d/pointerRadius;const force=s*s*1.25;n.vx+=(dx/d)*force;n.vy+=(dy/d)*force;}}
  ...
  return <div className="pointer-events-none absolute inset-0 overflow-hidden"><canvas ref={canvasRef} className="absolute inset-0 hidden md:block"/><div className="absolute inset-0 hud-grid opacity-70"/><div className="absolute inset-x-0 top-0 h-40 bg-gradient-to-b from-void to-transparent"/><div className="absolute inset-x-0 bottom-0 h-56 bg-gradient-to-t from-void to-transparent"/><div aria-hidden className="absolute inset-x-0 h-24 bg-gradient-to-b from-transparent via-teal/[0.06] to-transparent animate-scan hidden md:block"/></div>;
}
